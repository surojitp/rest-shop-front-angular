...
export class CatService implements OnInit {
  ...
  get_category(): Observable < any > {
    ...
    const url = 'https://shopbackend.herokuapp.com/category';
    return this.http.get(url);
  }
  get_category_with_sub_category(): Observable < any > {
    const url = 'https://shopbackend.herokuapp.com/category/categryWithSub';
    return this.http.get(url);
  }
  get_product_by_category_id(categoryId): Observable < any > {
    const url = 'https://shopbackend.herokuapp.com/products/byCategory/' + categoryId;
    return this.http.get(url);
  }
  get_product_by_subCategory(subCategoryId): Observable < any > {
    const url = 'https://shopbackend.herokuapp.com/products/bySubCategory/' + subCategoryId;
    return this.http.get(url);
  }
  get_product_details(productId): Observable < any > {
    const url = `https://shopbackend.herokuapp.com/products/${productId}`;
    return this.http.get(url);
  }
  ...
  product_add_to_cart(product) {
    ...
    if (Object.keys(this.loginData).length === 0) {
      console.log(1111111);
      if (this.getCookie('cart')) {
        const cart_array = JSON.parse(this.getCookie('cart'));
        this.cart = cart_array;
      }
      if (this.getCookie('total')) {
        const cart_total = this.getCookie('total');
      ...
      const pro_string = JSON.stringify(product);
  ...
  product_add_to_cart_when_login() {
    ...
    if (Object.keys(this.loginData).length !== 0) {
      ...
      const cartCookie = (this.getCookie('cart')) ? JSON.parse(this.getCookie('cart')) : [];
      if (cartCookie.length > 0) {
        /////////////////
        for (let i = 0; i < cartCookie.length; i++) {
          cartCookie[i]['userId'] = this.loginData.id;
          const product_data = JSON.stringify(cartCookie[i]);
  ...
  product_add_to_cart_service(product_data, token): Observable < any > {
    ...
    const url = 'https://shopbackend.herokuapp.com/cart';
  ...
  get_data_from_cart_service() {
    ...
    const headers = new Headers({
      'Content-Type': 'application/json',
      'Authorization': 'Barer ' + this.loginData.token
    });
    // headers.append('Authorization', 'Barer '+data.token)
    const options = new RequestOptions({
      headers: headers
    });
    const url = `https://shopbackend.herokuapp.com/cart/${this.loginData.id}`;
    return this.http.get(url)
      .pipe(
        map((response: any) => {
  ...
  removeItemCart(product) {
    ...
    const cart_array: any = JSON.parse(this.getCookie('cart'));
    this.total = JSON.parse(this.getCookie('total'));
    console.log('cartttt', this.total);
    // var index = cart_array.indexOf(product);
    const index = cart_array.map(function (pro) {
      return pro.id;
    }).indexOf(product.id);
    console.log('in', index);
    const cart_product_array = cart_array[index];
  ...
  remove_cart_service(product_id) {
    ...
    const url = 'https://shopbackend.herokuapp.com/cart/' + product_id;
